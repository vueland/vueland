...
Arbitrary classes are generated by a static source scan: `color="#7C4DFF"` and `:color="'#7C4DFF'"` work, while `:color="someVar"` with a raw value does not. Palette tokens are unaffected — their classes are always present in the CSS. See [`colorAttributes`](/en/plugins/utils-jit/configuration#colorattributes) for details.
:::
