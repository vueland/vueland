...
Arbitrary classes (`bg-[#fa5a5a]`) are generated by [utils-jit](/en/plugins/utils-jit/getting-started) via a static source scan: `color="#fa5a5a"` works, `:color="someVar"` with a raw value does not. Palette tokens are unaffected. See [Custom attrs](/en/plugins/utils-jit/custom-attrs) for details.
:::
