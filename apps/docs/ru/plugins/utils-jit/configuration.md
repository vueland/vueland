...
| `colorAttributes` | `string[]`                | `['color']` при установленном `@vueland/ui`, иначе `[]`      | Атрибуты/пропы, значения которых сканируются как цвета. См. [`colorAttributes`](#colorattributes).                                                         |
...
Компоненты `@vueland/ui` с пропом `color` строят utility-класс в рантайме: `color="#fa5a5a"` превращается в `bg-[#fa5a5a]` или `color-[#fa5a5a]` в зависимости от компонента и варианта. Статический скан такие классы в исходниках не находит — в коде есть только значение цвета, а не готовый класс.

`colorAttributes` закрывает этот разрыв: для перечисленных атрибутов сканер берёт значение и сам добавляет кандидатов `bg-[значение]` и `color-[значение]`.
...
```css
/* в virtual:utils-jit.css появятся оба кандидата */
.bg-\[\#7C4DFF\] {
...
**Дефолт автоматический:** если в проекте установлен `@vueland/ui` (в `dependencies`/`devDependencies` или резолвится из root), плагин включает `['color']` сам — настраивать ничего не нужно. Явно переданное значение (в том числе `[]`) отключает автодетект:

```ts
utilsJIT({
  // свои цветонесущие пропы в дополнение к color
  colorAttributes: ['color', 'track-color'],
})
```

Правила те же, что и для классов:

- разворачиваются только **сырые** значения: `#hex`, `rgb()/hsl()/oklch()/...`, `var(...)`. Палитровые токены (`color="red-lighten-2"`) в JIT не нуждаются — классы `bg-red-lighten-2`/`text-red-lighten-2` уже есть в статических утилитах `@vueland/ui`;
- значение должно быть литералом: `color="#fa5a5a"` и `:color="'#fa5a5a'"` сканер видит, `:color="someVar"` — нет (как и динамическая сборка классов);
- пробелы внутри значения нормализуются: `rgb(255, 90, 90)` и в классе, и в кандидате станет `rgb(255,90,90)`.
...
Кроме классов, сканируются значения атрибутов из [`colorAttributes`](#colorattributes) — по умолчанию `color`-пропы компонентов `@vueland/ui`.
