...
Дефолтные имена брейкпоинтов (`xs/sm/md/lg/xl/xxl`) — общий контракт: `@vueland/ui` генерит из них предопределённые `.md\:pa-4`, `.lg\:d-flex` и встроенные адаптивные пропсы `CRow`/`CCol`, а `useDisplay` отдаёт их в рантайм. Переопределение одного значения (например `{ sm: 680 }`) не должно ронять остальные имена — иначе вся библиотека лишилась бы `.md\:*` / `.lg\:*` утилит. Поэтому передать можно переопределение значений и **новые** имена (например `tablet: 1280`), но убрать дефолтные имена нельзя.
:::
...
### Кастомные имена брейкпоинтов в grid-компонентах
