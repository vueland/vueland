...
Эта страница описывает встроенные utility-классы `@vueland/utils-jit` и допустимые значения. Встроенные правила — это arbitrary-value утилиты, не привязанные к фреймворку; проектные статические и параметризованные утилиты добавляются через [`defineRule`](./custom-rules).
...
Классы вроде `flex-center`, `surface-[#fff]` или `grid-cols-3` не входят в дефолтный набор, пока вы не опишете их в `rules`. Примеры arbitrary, static и parameterized utilities есть в [пользовательских правилах](./custom-rules).
