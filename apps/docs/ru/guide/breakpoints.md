...
- **Компоненты сетки** — `CRow` и `CCol` используют те же значения в CSS сетки и встроенных адаптивных пропах (например, `md="6"`, `align-lg="center"`); кастомные имена брейкпоинтов используются через сгенерированные классы вроде `tablet-6`

Подробнее про компоненты — в разделе [Сетка](/ru/components/CRow).

По умолчанию каждый слой имеет встроенные значения. При настройке кастомных брейкпоинтов значения синхронизируются автоматически. Кастомные имена для сетки генерируют CSS-классы, а не дополнительные Vue-пропы.
...
Это запускает SCSS-препроцессор Vite, в ходе которого плагин перехватывает компиляцию и подставляет ваши значения в переменную `$grid-breakpoints`. В результате предопределённые утилиты (`sm:d-flex`), JIT-классы, media-запросы компонентов сетки и `useBreakpoints` работают с одними и теми же числами.
...
Всё подключено. Отзывчивые утилиты, CSS сетки, встроенные пропы компонентов сетки и `useBreakpoints` реагируют на одни и те же значения брейкпоинтов.
...
Кастомные имена брейкпоинтов из `utilsJIT` не создают новые пропы компонентов. Для таких имён используйте сгенерированные классы:

```html
<c-row>
  <c-col cols="12" class="tablet-6">Половина с tablet</c-col>
  <c-col cols="12" class="tablet-4">Треть с tablet</c-col>
</c-row>
```
...
Настройте брейкпоинты один раз в `vite.config.ts` — плагин синхронизирует утилитарные классы, CSS сетки, встроенные grid-пропы, JIT-классы и `useBreakpoints` автоматически. Подробности — в [Шаге 2](#шаг-2-настройте-vite) выше.
...
| `utilsJIT({ breakpoints })`        | управляет SCSS-утилитами, CSS/классами сетки, значениями встроенных grid-пропов, `useBreakpoints` и JIT-классами |
