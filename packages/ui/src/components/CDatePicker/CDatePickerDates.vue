<script setup lang="ts">
    ...
    const weekDays = computed(() => unref(days).map((d) => ({ day: d, label: props.locale?.[d] })))
    ...
    const parsedFromTo = computed(() => {
        ...
        return { f: parseDate(new Date(from)).mls ?? 0, t: parseDate(new Date(to)).mls ?? 0 }
    })
    ...
    function isDisabled(date: DatePickerDate): boolean {
        ...
        const { disabledDates: d, minDate, maxDate } = props
        return [
    ...
    const enrichedDates = computed<EnrichedDate[]>(() =>
        dates.value.map((dateObj) => {
            if (!dateObj || !dateObj.date) return { dateObj, empty: true, disabled: false, highlighted: false, isSelected: false, isToday: false }
